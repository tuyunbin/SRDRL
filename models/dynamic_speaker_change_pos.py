...
class DynamicCore(nn.Module):
    def __init__(self, cfg):
        ...
        self.gate1x = nn.Sequential(
            nn.Linear(cfg.model.speaker.rnn_size * 2 + cfg.model.speaker.input_dim,
                      cfg.model.speaker.rnn_size * 2 + cfg.model.speaker.input_dim),
            nn.ReLU(),
            nn.Dropout(self.drop_prob_lm)
        )

        self.gate2x = nn.Linear(cfg.model.speaker.rnn_size*2 + cfg.model.speaker.input_dim,
    ...
    def forward(self, xt, loc_feat_bef, loc_feat_aft,
                feat_diff, state):
...
class DynamicSpeaker(CaptionModel):
    ...
    def _forward(self,
        ...
        for i in range(self.seq_length):
            ...
            output, state, output_pos = self.get_logprobs_state(it,
                                                    feat_bef, feat_aft,
                                                    feat_diff, state)
            outputs[:, i] = output
    ...
    def get_logprobs_state(self, it,
        ...
        output, state, output_pos = self.core(xt,
                                  feat_bef, feat_aft,
                                  feat_diff, state)
    ...
    def _sample(self,
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state, log_pos_probs = self.get_logprobs_state(it,
                                                      feat_bef, feat_aft,
                                                      feat_diff, state)
