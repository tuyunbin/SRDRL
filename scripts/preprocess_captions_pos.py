...
from utils.preprocess import tokenize, encode, build_vocab, filt_word_category
...
parser.add_argument('--input_pos', default='')
...
def main(args):
    ...
    with open(args.input_pos, 'rb') as fo:
        pos_token = pickle.load(fo)

    filted_class, words_class, class_id, unmasked_classid = filt_word_category(pos_token, word_to_idx)
    ...
    for img, tokens_list in all_cap_tokens:
        ...
        Pi = np.zeros((n, max_length), dtype=np.int)
        for j, tokens in enumerate(tokens_list):
            ...
            pos_encoded = encode(tokens, filted_class, allow_unk=args.allow_unk == 1)
            for k, w in enumerate(tokens_encoded):
                Li[j, k] = w
            for k, w in enumerate(pos_encoded):
                Pi[j, k] = w
        # captions are padded with zeros
        label_arrays.append(Li)
        pos_arrays.append(Pi)
    ...
    P = np.concatenate(pos_arrays, axis=0)  # put all pos together
    ...
    neg_P = np.concatenate(neg_pos_arrays, axis=0) # put all pos together
    ...
    with h5py.File(args.output_h5, 'w') as f:
        ...
        f.create_dataset('neg_pos', data=neg_P)
