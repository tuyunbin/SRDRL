...
class RCCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        neg_pos = np.zeros([self.seq_per_img, self.max_seq_length + 1], dtype=int)
        if n_cap < self.seq_per_img:
            # we need to subsample (with replacement)
            for q in range(self.seq_per_img):
                ixl = random.randint(ix1, ix2)
        ...
        return (d_feature, n_feature, q_feature,
                seq, neg_seq, pos, neg_pos, mask, neg_mask, aux_label_pos, aux_label_neg,
...
def rcc_collate(batch):
    ...
    pos_batch = default_collate(transposed[5])
    neg_pos_batch = default_collate(transposed[6])
    ...
    return (d_feat_batch, n_feat_batch, q_feat_batch,
            seq_batch, neg_seq_batch, pos_batch, neg_pos_batch,
