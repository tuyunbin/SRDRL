...
from models.modules_SRDRL import ChangeDetector, AddSpatialInfo
...
while t < cfg.train.max_iter:
    ...
    for i, batch in enumerate(train_loader):
        ...
        labels, no_chg_labels, sc_pos_labels, nsc_pos_labels, masks, no_chg_masks, aux_labels_pos, aux_labels_neg, \
        ...
        sc_pos_labels, nsc_pos_labels = sc_pos_labels.to(device), nsc_pos_labels.to(device)
        d_feats, nsc_feats, sc_feats = \
        ...
        speaker_output_neg, speaker_output_neg_tag = speaker._forward(chg_neg_feat_bef,
                                              chg_neg_feat_aft,
                                              chg_neg_feat_diff,
                                              no_chg_labels)
        ...
        speaker_pos_loss = 0.5 * lang_criterion(speaker_output_pos_tag, sc_pos_labels[:, 1:], masks[:, 1:]) + \
                       0.5 * lang_criterion(speaker_output_neg_tag, nsc_pos_labels[:, 1:], no_chg_masks[:, 1:])
        ...
        total_loss = speaker_loss + speaker_pos_loss + 2.5e-03 * att_sum + entropy_loss
        ...
        if t % cfg.train.snapshot_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_i, val_batch in enumerate(val_loader):
                    d_feats, nsc_feats, sc_feats, \
                    labels, no_chg_labels, sc_pos_feats, nsc_pos_feats, masks, no_chg_masks, aux_labels_pos, aux_labels_neg, \
                    d_img_paths, nsc_img_paths, sc_img_paths = val_batch

                    val_batch_size = d_feats.size(0)

                    d_feats, nsc_feats, sc_feats = d_feats.to(device), nsc_feats.to(device), sc_feats.to(device)
                    sc_pos_feats, nsc_pos_feats = sc_pos_feats.to(device), nsc_pos_feats.to(device)
                    d_feats, nsc_feats, sc_feats = \
                        spatial_info(d_feats), spatial_info(nsc_feats), spatial_info(sc_feats)
                    labels, masks = labels.to(device), masks.to(device)
                    no_chg_labels, no_chg_masks = no_chg_labels.to(device), no_chg_masks.to(device)
